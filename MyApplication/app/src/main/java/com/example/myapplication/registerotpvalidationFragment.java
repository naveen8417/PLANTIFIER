...
public class registerotpvalidationFragment extends Fragment {
    ...
    private void ButtonClicks() {
        confrmotp_verify.setOnClickListener(view -> {
            CommonMethods.DisplayShortTOAST(thiscontext, "Verifying");
            otp = Objects.requireNonNull(confrmotp_otpedittext.getText()).toString().trim();
            if (otp.length() == 0) {
        ...
        confrmotp_resend.setOnClickListener(view -> sendotptophone(phone));
        confrmotp_gobckbot.setOnClickListener(view -> requireActivity().getSupportFragmentManager().beginTransaction().replace(R.id.startupviewcontainer, new registerFragment()).addToBackStack(null).commit());
    }
    ...
    private void verifyphoneotp(String otp, String backendotp) {
        if (backendotp != null && otp != null) {
            ...
            mAuth.signInWithCredential(credential)
                    ...
                    }).addOnFailureListener(e -> CommonMethods.DisplayShortTOAST(thiscontext, e.getMessage()));
        } else {
    ...
    private void RegisterUser(String name, String email, String phone, String passwordConfirmed, String password) {
        ...
        APICall.okhttpmaster().newCall(
            ...
            public void onFailure(@NonNull Call call, @NonNull IOException e) {
                requireActivity().runOnUiThread(() -> CommonMethods.DisplayLongTOAST(thiscontext, e.getMessage()));
            }
            ...
            public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException {
                ...
                requireActivity().runOnUiThread(() -> {
                    try {
                        ...
                        if (Objects.equals((responsez).optString("status"), "fail")) {
                            ...
                            String name1 = Signup.optString("name");
                            String email1 = Signup.optString("email");
                            String phone1 = Signup.optString("phone");
                            String role = Signup.optString("role");
                            String id = Signup.optString("_id");
                            Savedatatoprefs(token, name1, email1, phone1, role, id);
                            requireActivity().getSupportFragmentManager().beginTransaction().replace(R.id.startupviewcontainer, new registerconformationFragment()).commit();
                        }
